...
void FastFinRL::fill_market_batch(
    ...
    // Parallel fill all samples
    tbb::parallel_for(tbb::blocked_range<size_t>(0, N),
        [=, this](const tbb::blocked_range<size_t>& range) {
            for (size_t s = range.begin(); s < range.end(); ++s) {
                size_t global_idx = samples[s].first;
                int day = samples[s].second;

                // Get first_day for this ticker (need to check if data available)
                int first_day = 0;
                if (global_idx < ticker_row_table_.size() && !ticker_row_table_[global_idx].empty()) {
                    // Find first valid day
                    for (int d = 0; d < max_day_; ++d) {
                        if (ticker_row_table_[global_idx][d] != static_cast<size_t>(-1)) {
                            first_day = d;
                            break;
                        }
                    }
                ...
                // Fill time_len data points for this sample
                for (int i = 0; i < time_len; ++i) {
                    ...
                    if (d >= first_day && d >= 0 && d < max_day_ &&
                        global_idx < ticker_row_table_.size()) {
                        size_t row_idx = ticker_row_table_[global_idx][d];
                        ...
                        if (row_idx != static_cast<size_t>(-1)) {
                            ...
                            if (mask_out) mask_out[out_idx] = 1;
                            continue;
                        }
                    }
                    ...
                    if (mask_out) mask_out[out_idx] = 0;
                }
            }
        }
    );
